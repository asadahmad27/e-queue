...
const Select = forwardRef<HTMLSelectElement, SelectProps>(
  (
    ...
    <FormField
      {...{ className, label, helpText, error, variant }}
      isRequired={intrinsicSelectProps.required}
      isDisabled={intrinsicSelectProps.disabled}
    >
      {(baseClasses) => (
        <div className="relative">
          <ChevronDownIcon className="absolute top-0 right-0 mt-[0.9rem] mr-2 w-5 text-true-gray-500 select-none pointer-events-none" />
          <select
            ref={ref}
